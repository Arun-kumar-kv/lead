...
"""
Auto-reflected models for Revenue & Rent tables.
eq_ls_contract_charges_view is a DB view with no PK —
reflected via Table() and mapped imperatively with an explicit
primary_key declaration.
"""
...
def _get_class(table_name: str):
    """Finds automap class handling schema-prefixed keys like 'public.eq_ls_contract'."""
...
# Contracts
try:
    EqLsContract        = _get_class("eq_ls_contract")
    EqLsPaymentTerms    = _get_class("eq_gen_payment_terms")
    EqLsContractCharges = _get_class("eq_ls_contract_charges")
except AttributeError as e:
    logger.error(f"Contract model not found: {e}")
    raise
...
# Accounting
try:
    EqAccVoucher            = _get_class("eq_acc_voucher_offset")
    EqAccVoucherTransaction = _get_class("eq_acc_voucher_transaction")
    EqAccChartLedgers       = _get_class("eq_acc_chart_of_acc_ledgers")
    EqAccTenantReceipt      = _get_class("eq_acc_tenant_receipt")
    EqAccAmountType         = _get_class("eq_acc_amount_type")
    logger.info("Accounting models reflected successfully")
except AttributeError as e:
    logger.error(f"Accounting model not found: {e}")
    raise
