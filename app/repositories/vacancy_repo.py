...
class VacancyRepository:
    ...
    Source tables:
    - eq_ls_property_unit         -> EqLsPropertyUnit
    - eq_ls_property              -> EqLsProperty
    - eq_ls_property_unit_status  -> EqLsPropertyUnitStatus
    - eq_ls_property_unit_type    -> EqLsPropertyUnitType
    """
    ...
    def get_total_units_live(self) -> int:
        """Total units in system"""
        return (
            self.db.query(func.count(EqLsPropertyUnit.ID))
            .scalar()
            or 0
        )
    ...
    def get_occupied_units_live(self) -> int:
        ...
        return (
            ...
            .filter(EqLsPropertyUnitStatus.STATUS == "Leased")
            .scalar()
            or 0
        )
    ...
    def get_booked_units_live(self) -> int:
        ...
        return (
            ...
            .filter(
                EqLsPropertyUnitStatus.STATUS.in_(
                    ["Booked", "Reserved", "Temporarily Booked"]
                )
            )
            .scalar()
    ...
    def get_rent_ready_unleased_live(self) -> int:
        """Rent-ready unleased units (STATUS = 'Available')"""
        return (
            self.db.query(func.count(EqLsPropertyUnit.ID))
            .join(
                EqLsPropertyUnitStatus,
    ...
    def get_maintenance_downtime_units_live(self) -> int:
        """Units blocked due to maintenance (not implemented in status-driven model)"""
        return 0

    def get_avg_vacancy_days_live(self) -> float:
        """
        Average days units have been vacant based on LAST_UPDATED_AT.
        Uses PostgreSQL-compatible date diff instead of MySQL's DATEDIFF.
        """
        result = (
            self.db.query(
                func.avg(
                    func.extract(
                        'day',
                        func.current_date() - EqLsPropertyUnit.LAST_UPDATED_AT
                    )
                )
            )
            .join(
            ...
            .filter(
                ...
                EqLsPropertyUnit.LAST_UPDATED_AT.isnot(None)
            )
            .scalar()
        )
        return round(float(result), 2) if result else 0.0
    ...
    def get_vacancy_by_property_live(self) -> List[Dict[str, Any]]:
        ...
        results = (
            self.db.query(
                func.trim(EqLsProperty.NAME).label("property_name"),

                func.sum(
                    case((EqLsPropertyUnitStatus.STATUS == "Leased", 1), else_=0)
                ).label("occupied_units"),

                func.sum(
                    case((EqLsPropertyUnitStatus.STATUS == "Available", 1), else_=0)
                ).label("vacant_units"),
            ...
            .join(EqLsPropertyUnit,
                  EqLsProperty.ID == EqLsPropertyUnit.PROPERTY_ID)
            .join(EqLsPropertyUnitStatus,
                  EqLsPropertyUnit.STATUS == EqLsPropertyUnitStatus.ID)
            .group_by(EqLsProperty.ID, EqLsProperty.NAME)
            .order_by(
                text("vacant_units DESC"),
                EqLsProperty.NAME.asc()
            )
            .all()
        )
    ...
    def get_vacancy_by_unit_type_live(self) -> List[Dict[str, Any]]:
        ...
        results = (
            self.db.query(
                ...
                func.count(EqLsPropertyUnit.ID).label("total_units"),

                func.sum(
                    case(
                        (EqLsPropertyUnitStatus.STATUS == "Available", 1),
                        else_=0
                    )
                ).label("vacant_units"),
                ...
                func.sum(
                    case(
                        (EqLsPropertyUnitStatus.STATUS == "Leased", 1),
                        else_=0
                    )
                ).label("occupied_units"),
            ...
            .join(
                EqLsPropertyUnitType,
                EqLsPropertyUnit.UNIT_TYPE == EqLsPropertyUnitType.ID
            )
            .group_by(EqLsPropertyUnitType.NAME)
            .order_by(EqLsPropertyUnitType.NAME)
            .all()
        )
    ...
    def get_vacancy_duration_buckets_live(self) -> Dict[str, int]:
        """
        Vacancy buckets based on LAST_UPDATED_AT.
        Uses PostgreSQL-compatible date arithmetic instead of MySQL's DATEDIFF.
        """
        days_vacant = func.extract(
            'day',
            func.current_date() - EqLsPropertyUnit.LAST_UPDATED_AT
        )
        ...
        results = (
            self.db.query(
                case(
                    (days_vacant.between(0, 30),  "0-30 days"),
                    (days_vacant.between(31, 60), "31-60 days"),
                    (days_vacant.between(61, 90), "61-90 days"),
                    else_="90+ days"
                ).label("vacancy_bucket"),
                func.count().label("total")
            )
