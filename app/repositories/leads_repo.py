...
class LeadsRepository:
    ...
    def get_vacant_units_lead_coverage(self, filters: dict = None) -> Dict[str, Any]:
        ...
        leads_conditions = [
            EqLsLeads.UNIT_ID == EqLsPropertyUnit.ID,
            EqLsLeads.CREATED_AT >= cutoff,
        ]
        if filters:
            if filters.get("property_id"):
                leads_conditions.append(EqLsLeads.PROPERTY_ID == filters["property_id"])
            if filters.get("property_type"):
                leads_conditions.append(EqLsLeads.LEADS_TYPE == filters["property_type"])
            if filters.get("date_from"):
                leads_conditions.append(EqLsLeads.INQUIRY_DATE >= datetime.strptime(filters["date_from"], "%Y-%m-%d"))
            if filters.get("date_to"):
                date_to = datetime.strptime(filters["date_to"], "%Y-%m-%d").replace(hour=23, minute=59, second=59)
                leads_conditions.append(EqLsLeads.INQUIRY_DATE <= date_to)

        query = (
            self.db.query(
                func.count(distinct(EqLsPropertyUnit.ID)).label("total_vacant_units"),
                func.count(distinct(EqLsLeads.UNIT_ID)).label("vacant_units_with_leads"),
                func.count(EqLsLeads.ID).label("total_leads_on_vacant"),
            )
            ...
            .outerjoin(EqLsLeads, and_(*leads_conditions))
